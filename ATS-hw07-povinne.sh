...
if [ -e $PUBLIC_PATH ];then
    ...
    do
        if [ $i -le 9 ];then
            PUB_FILE=$PUBLIC_PATH/pub0"$i"-m
        else
            PUB_FILE=$PUBLIC_PATH/pub"$i"-m
        fi

        if [ -f $PUB_FILE.in ];then
            $TESTED_PROGRAM $(more $PUB_FILE.in.pat) $PUB_FILE.in >$PUB_FILE-my.out 2> $PUB_FILE-my.err
            echo $? >$PUB_FILE-my.in.return
            if [ $VERBOSE -ge 1 ];then
                ...
                echo "Comaparing return values ($PUB_FILE.in.return $PUB_FILE-my.in.return) :"
                DIFFERNCE=$(diff $PUB_FILE.in.return $PUB_FILE-my.in.return)
                if [ "$DIFFERNCE" != "" ]; then
                    echo $DIFFERNCE
                    FOUND_ISSUES=$(($FOUND_ISSUES+1))
                fi
